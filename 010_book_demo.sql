...
begin
  ...
  if p_preferred_date is null or p_preferred_date < (current_date + 1) then
  ...
  if exists (select 1 from public.demo_requests where email = v_email) then
    raise exception 'A demo request already exists for this email.' using errcode = '23505';
  end if;
