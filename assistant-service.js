...
function localAnswer(question, snapshot) {
  ...
  if (marketing && /(promot|advertis|best product|best customer|bundle|repeat customer|customer segment|marketing)/.test(q)) {
  ...
  const attention = approvals.length + overdue.length + followUps.length;
  if (q.includes('attention') || q.includes('urgent') || q.includes('today')) {
    return {
      answer: attention ? `I found ${attention} item${attention === 1 ? '' : 's'} worth your attention: ${approvals.length} approval${approvals.length === 1 ? '' : 's'}, ${followUps.length} customer follow-up${followUps.length === 1 ? '' : 's'}, and ${overdue.length} overdue task${overdue.length === 1 ? '' : 's'}.` : 'Nothing needs your attention right now. You are all caught up.',
  ...
  if (q.includes('follow')) {
    const names = followUps.slice(0, 5).map(customer => customer.name).join(', ');
    return { answer: names ? `${names} ${followUps.length === 1 ? 'may need' : 'may need'} a follow-up based on their status or last contact date.` : 'I could not find any customers that currently need a follow-up.', next_steps: names ? ['Open Customers', 'Prepare a draft for review'] : ['Add last-contact dates to customer records'], sources: ['Customers'], mode: 'local' };
  }
  if (q.includes('activity') || q.includes('did elio') || q.includes('recent')) {
    const recent = activities.slice(0, 3).map(item => item.action).join(' • ');
  ...
  if (q.includes('task')) {
    return { answer: `You have ${tasks.filter(task => task.status !== 'Completed').length} active task${tasks.filter(task => task.status !== 'Completed').length === 1 ? '' : 's'} and ${overdue.length} overdue. I can help you decide what to do next, but I will not change data without your confirmation.`, next_steps: ['Review Tasks', 'Create a task manually'], sources: ['Tasks'], mode: 'local' };
  }
