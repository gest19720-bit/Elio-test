...
function wireMobileMenu() {
  ...
  const setOpen=open=>{
    ...
    if(open){
      ...
      requestAnimationFrame(()=>requestAnimationFrame(()=>{backdrop.classList.add('open');sheet.classList.add('open');focusable()[0]?.focus();}));
    } else {
...
function followUpModal(b,customer){const wrap=document.createElement('div');wrap.className='modal-backdrop';wrap.innerHTML=`<div class="modal"><div class="modal-header"><div><span class="eyebrow">Elio follow-up</span><h2 style="margin-top:5px">Prepare something thoughtful.</h2><p>For ${esc(customer.name)}</p></div><button class="btn btn-quiet" data-close>×</button></div><form><div class="field"><label>Context</label><textarea name="context" placeholder="What should Elio know about this follow-up?" required></textarea></div><div class="field"><label>Tone</label><select name="tone"><option>Friendly</option><option>Professional</option><option>Casual</option><option>Formal</option></select></div><button class="btn btn-primary">${window.ELIO_AI_MODE==='mock'?'Prepare draft (mock mode)':'Prepare follow-up'}</button><p class="muted" style="margin-top:12px;font-size:12px">Nothing will be sent automatically. You will review it first.</p></form></div>`;document.body.append(wrap);wrap.querySelector('[data-close]').onclick=()=>wrap.remove();wrap.querySelector('form').onsubmit=async e=>{e.preventDefault();const f=e.target,btn=f.querySelector('button');btn.disabled=true;btn.textContent='Elio is preparing this…';try{const result=await generateFollowUp({customer,customerId:customer.id,context:f.context.value.trim(),tone:f.tone.value});await createFollowUpWorkflow({businessId:b.id,customerId:customer.id,subject:result.subject,message:result.message,suggestedAction:result.suggested_action,generatedBy:result.mode==='mock'?'Elio (mock mode)':'Elio'});wrap.remove();toast(result.mode==='mock'?'Draft created in local AI test mode and sent to approvals.':'Draft created for approval.');}catch(error){btn.disabled=false;btn.textContent='Try again';toast(friendlyError(error,'Elio could not save this follow-up.'))}};}
