...
//
// Setting `wrap.dataset.locked` makes a modal resist Escape and backdrop
// dismissal. The MCP token flows lock while creation is pending and while
// the one-time token is on screen — closing then would discard the only
// displayed copy of a secret that cannot be shown again.

let openCount = 0;

export function openModal(wrap) {
  if (!wrap) return wrap;
  const onKeydown = e => { if (e.key === 'Escape' && !wrap.dataset.locked) wrap.remove(); };
  const release = () => {
  ...
  wrap.addEventListener('click', e => { if (e.target === wrap && !wrap.dataset.locked) wrap.remove(); });
