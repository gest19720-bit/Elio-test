...
export function calculateMetrics(data) {
  ...
  return { products, customers, tasks, activities, activeTasks, lowStock: products.filter(item => Number(item.stock) <= 5), topProducts: [...products].sort((a, b) => Number(b.sales || 0) - Number(a.sales || 0)), recordedSales, recordedSalesValue, stockUnits: sum(products.map(item => item.stock)), currentCustomers, previousCustomers, totalCustomerTotal, currentCustomerTotal, previousCustomerTotal, currentTasks, previousTasks, currentActivities, previousActivities, comparisons: { customers: percentageChange(currentCustomerTotal, previousCustomerTotal), tasks: percentageChange(currentTasks.length, previousTasks.length), activity: percentageChange(currentActivities.length, previousActivities.length) } };
}
