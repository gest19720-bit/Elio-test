const { AUTHORIZATION_REQUEST_TTL_SECONDS, AUTHORIZATION_CODE_TTL_SECONDS, origin, oauthError, sendJson, sendOAuthError, secret, hash, nowPlus, parseScope, requestBody, supabase, authenticatedUser, resolveClient, validateRedirect, requirePkce, cors } = require('./_shared');
...
module.exports = async (req, res) => {
  try {
    if (req.method === 'GET') {
      ...
      return res.redirect(302, `${origin(req)}/app/oauth-consent.html?request_id=${encodeURIComponent(requestId)}`);
    }
