...
function topics(question: string) {
  ...
  return {
    ...
    customers: /customer|lead|follow.?up|client|sales|revenue|marketing|performance/.test(value), products: /product|inventory|stock|catalog|catalogue|sell|sellers?|selling|best.?seller|promote|promotion|price|pricing|margin|sales|revenue|marketing|performance/.test(value),
  ...
  try {
    ...
    const [tasks, approvals, customers, products, activities] = await Promise.all([
      ...
      use.products ? (async () => {
        ...
        for (const row of [...(recent.data || []), ...(topSelling.data || []), ...(lowStock.data || [])]) {
          if (row?.id) { if (seen.has(row.id)) continue; seen.add(row.id); }
          merged.push(row);
        }
        merged.sort((a: any, b: any) => (Number(b?.sales) || 0) - (Number(a?.sales) || 0) || (Number(a?.stock) || 0) - (Number(b?.stock) || 0));
        return { data: merged.slice(0, 40), error: recent.error || topSelling.error || lowStock.error };
